...
| Product Hunt 账号 | 获取 API Token | 可选，用于提升请求限制 |
...
1. 在飞书中新建多维表格，命名为 `Product Hunt 监控`
...
2. 选择「自定义机器人」，命名为 `Product Hunt 监控助手`
3. 复制并保存 Webhook URL：
   ```
...
5 8 * * * cd /opt/cortexcrawl/producthunt-workflow && \
  /opt/cortexcrawl/producthunt-workflow/venv/bin/python src/main.py \
  >> /var/log/producthunt-monitor.log 2>&1
```
...
triggers:
  - type: schedule
    cron: "0 8 * * *"   # UTC 08:00，即北京时间 16:00
    timezone: "UTC"
```

OpenClaw 完成抓取后，会自动通过 Webhook 触发 Python 工作流（`http://localhost:8000/api/process-producthunt-data`）。
...
```
2026-03-20 16:00:00 - INFO - 🚀 Product Hunt 监控工作流启动
2026-03-20 16:00:01 - INFO - 📂 加载数据: /data/producthunt/raw_products.json
2026-03-20 16:00:02 - INFO - 📊 从飞书多维表格加载现有数据...
2026-03-20 16:00:04 - INFO - 🔄 处理数据 (去重但更新)...
2026-03-20 16:00:05 - INFO - 处理完成: 新增 5 个产品, 更新 20 个产品, 未变更 125 个产品
2026-03-20 16:00:09 - INFO - 📤 发送飞书通知...
2026-03-20 16:00:10 - INFO - 卡片消息发送成功
```
...
# 过滤错误
grep -i "error\|exception" /var/log/producthunt-monitor.log
```
...
| `field type mismatch` | 表格字段类型不匹配 | 检查「投票数」「评论数」是否为数字类型，「发布日期」是否为日期类型 |
| 数据重复写入 | 产品链接格式不一致 | 确认 `product_url` 不含追踪参数，格式统一为 `https://www.producthunt.com/posts/xxx` |
| `403` 抓取失败 | Product Hunt API 频率限制 | 配置 `PRODUCT_HUNT_API_TOKEN`，或增加抓取间隔 |
...
cd producthunt-workflow
source venv/bin/activate
pip install -r requirements.txt   # 如有新依赖

# 上传最新 Skill 文件到 OpenClaw
scp openclaw/* root@47.254.73.23:/opt/openclaw/skills/producthunt-monitor/
